...
xtreg lTurnoverGross year* Post Treat, cluster(DealerTIN) fe
...
xtreg MoneyDeposited AnnualYear* year* Post Treat, cluster(DealerTIN) fe
...
xtreg lTurnoverGross year* AnnualDummy Post Treat, cluster(DealerTIN) fe
xtreg lAggregate year* AnnualDummy Post Treat, cluster(DealerTIN) fe

xtreg Money year* AnnualDummy Post Treat, cluster(DealerTIN) fe
xtreg lMoney year* AnnualDummy Post Treat, cluster(DealerTIN) fe
...
------------------------------------------------------------------------------
             |               Robust
AggregateA~d |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
       year1 |          0  (omitted)
       year2 |    16630.4   2484.976     6.69   0.000     11759.89    21500.92
       year3 |  -10112.09   4337.873    -2.33   0.020    -18614.25   -1609.921
       year4 |          0  (omitted)
       year5 |   20482.59   3332.456     6.15   0.000     13951.03    27014.16
 AnnualDummy |          0  (omitted)
        Post |   58934.66   8600.441     6.85   0.000     42077.93    75791.39
       Treat |  -37473.73   8778.739    -4.27   0.000    -54679.93   -20267.54
       _cons |   131556.1   4051.653    32.47   0.000     123614.9    139497.3
-------------+----------------------------------------------------------------
     sigma_u |  1909052.2
     sigma_e |  1228017.8
         rho |  .70732162   (fraction of variance due to u_i)
------------------------------------------------------------------------------
...
Fixed-effects (within) regression               Number of obs      =    448917
Group variable: DealerTIN                       Number of groups   =    107118

R-sq:  within  = 0.0025                         Obs per group: min =         1
       between = 0.1771                                        avg =       4.2
       overall = 0.1095                                        max =         5

                                                F(5,107117)        =     95.80
corr(u_i, Xb)  = -0.3776                        Prob > F           =    0.0000

                         (Std. Err. adjusted for 107118 clusters in DealerTIN)
------------------------------------------------------------------------------
             |               Robust
lTurnoverG~s |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
       year1 |          0  (omitted)
       year2 |  -.0046554   .0028845    -1.61   0.107     -.010309    .0009982
       year3 |  -.0109742   .0037506    -2.93   0.003    -.0183254    -.003623
       year4 |   .0181867   .0032306     5.63   0.000     .0118548    .0245185
       year5 |          0  (omitted)
 AnnualDummy |          0  (omitted)
        Post |   .0017759   .0046356     0.38   0.702    -.0073099    .0108616
       Treat |    .147395   .0083901    17.57   0.000     .1309504    .1638395
       _cons |   15.39882   .0024234  6354.22   0.000     15.39407    15.40357
-------------+----------------------------------------------------------------
     sigma_u |  1.8312419
     sigma_e |  .73947788
         rho |  .85979767   (fraction of variance due to u_i)
------------------------------------------------------------------------------
...
Fixed-effects (within) regression               Number of obs      =    261933
Group variable: DealerTIN                       Number of groups   =     81057

R-sq:  within  = 0.0029                         Obs per group: min =         1
       between = 0.0005                                        avg =       3.2
       overall = 0.0001                                        max =         5

                                                F(5,81056)         =    104.24
corr(u_i, Xb)  = -0.0168                        Prob > F           =    0.0000

                          (Std. Err. adjusted for 81057 clusters in DealerTIN)
------------------------------------------------------------------------------
             |               Robust
lAggregate~d |      Coef.   Std. Err.      t    P>|t|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
       year1 |          0  (omitted)
       year2 |  -.0041304    .006367    -0.65   0.517    -.0166096    .0083487
       year3 |  -.1010753   .0075615   -13.37   0.000    -.1158958   -.0862547
       year4 |   -.149326   .0070606   -21.15   0.000    -.1631648   -.1354872
       year5 |          0  (omitted)
 AnnualDummy |          0  (omitted)
        Post |   .1288741   .0088495    14.56   0.000      .111529    .1462191
       Treat |   .0084381   .0140859     0.60   0.549    -.0191702    .0360465
       _cons |   10.52711   .0048381  2175.86   0.000     10.51762    10.53659
-------------+----------------------------------------------------------------
     sigma_u |  2.1016922
     sigma_e |  1.1005549
         rho |  .78479925   (fraction of variance due to u_i)
------------------------------------------------------------------------------
...
table AnnualDummy Post, c(mean lTurnoverGross)
